...
#define PIN_SWCLK  	GET_PIN(B, 13)
#define PIN_SWDIO  	GET_PIN(B, 14)
/*
 * BSRR TO 1
 * BRR  TO 0
 * */
...
{
    rt_pin_mode(PIN_SWCLK, PIN_MODE_OUTPUT);
    rt_pin_mode(PIN_SWDIO, PIN_MODE_OUTPUT);

    rt_pin_write(PIN_SWCLK, PIN_HIGH);
    rt_pin_write(PIN_SWDIO, PIN_HIGH);
}
...
{
    rt_pin_mode(PIN_SWCLK, PIN_MODE_INPUT);
    rt_pin_mode(PIN_SWDIO, PIN_MODE_INPUT);
}
...
{
    return rt_pin_read(PIN_SWCLK) ? 1 : 0;
}
...
{
    rt_pin_write(PIN_SWCLK, PIN_LOW);
}
...
{
    return rt_pin_read(PIN_SWDIO) ? 1 : 0;
}
...
{
    rt_pin_write(PIN_SWDIO, PIN_LOW);
}
...
{
    if(bit & 1) rt_pin_write(PIN_SWDIO, PIN_HIGH);
    else        rt_pin_write(PIN_SWDIO, PIN_LOW);
}
