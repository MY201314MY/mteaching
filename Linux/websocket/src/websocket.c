...
void websocket_listen(websocket_t* ws) {
    ...
    websocket_clients_t* clients = NULL;

    ws->clients = clients;

    websocket_util_verbose("WebSocket server is listening from 0.0.0.0:%d\n", ws->port);

    pthread_t client_thread;

    char str_addr[INET6_ADDRSTRLEN+1];

    websocket_clients_t* client = NULL;
    websocket_clients_t* current_client = NULL;
    ...
    for (;;) {
        ...
        websocket_util_verbose("Client connected: #%d (%s)\n", client->id, str_addr);
...
void websocket_send(websocket_clients_t* client, char* message) {
    websocket_util_verbose("Sending to #%d (%d): %s\n", client->id, client->address, message);
    
    unsigned int message_len = strlen(message);

    if (message_len < 126) {
        websocket_packet_frame_len8_t message_frame;
        message_frame.fin_rsv_opcode = 0b10000001;
        message_frame.mlen8 = message_len;

        send(client->socket, &message_frame, sizeof(websocket_packet_frame_len8_t), 0);
...
static void receive_http_packet(websocket_clients_t* client) {
    ...
    if (!result) {
        websocket_util_verbose("Client disconnected: %s\n", str_addr);
    ...
    if (parser_state == CEBSOCKET_HTTP_HEADER_PARSER_STATE_METHOD) {
        ...
        *(header_buff+(header_buff_i++)) = byte;
        ...
        if (byte == '\n') {
            ...
            accept = websocket_util_base64_encode(hash);
...
static void receive_ws_packet(websocket_clients_t* client) {
    ...
    websocket_util_verbose("Data from #%d: %s\n", client->id, req);
...
static void client_disconnected(websocket_clients_t* client) {
    ...
    websocket_util_verbose("Client disconnected: #%d (%s)\n", client->id, str_addr);
    client->ws->on_disconnected(client);
    websocket_client_free(client);
}

websocket_clients_t* websocket_client_init() {
    websocket_clients_t* client = malloc(sizeof(websocket_clients_t));
    client->socket = 0;
    client->ws_key = NULL;
    client->address = 0;
    client->data = NULL;
...
void broken_pipe_handler(int sig) {
    websocket_util_verbose("Broken pipe.\n");
}