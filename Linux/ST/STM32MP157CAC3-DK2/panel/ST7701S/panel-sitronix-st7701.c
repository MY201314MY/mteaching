...
{
	uint8_t PanelReadBuff = 0;
	const struct drm_display_mode *mode = st7701->desc->mode;
	
	printk("init sequence.\r\n");
	if(mipi_dsi_dcs_read(st7701->dsi, 0xDA, &PanelReadBuff, 1))
	{
		printk("ID1:0x%02X", PanelReadBuff);
	}
	if(mipi_dsi_dcs_read(st7701->dsi, 0xDB, &PanelReadBuff, 1))
	{
		printk("ID2:0x%02X", PanelReadBuff);
	}
	if(mipi_dsi_dcs_read(st7701->dsi, 0xDC, &PanelReadBuff, 1))
	{
		printk("ID3:0x%02X\n\n", PanelReadBuff);
	}
	...
	msleep(25);
	printk("Simple Panel ST7701S [MIPI] Experience.\r\n");
	if(mipi_dsi_dcs_read(st7701->dsi, 0xDA, &PanelReadBuff, 1))
	{
		printk("ID1:0x%02X", PanelReadBuff);
	}
...
static const struct st7701_panel_desc ts8550b_desc = {
	...
	.panel_sleep_delay = 120, /* panel need extra 120ms for sleep out cmd */
};
