...
extern DISPLAY SAMSUNG;
void LCD_WrDat(uint16_t y,uint16_t x,uint8_t data)
{
  uint8_t i=0;
  ...
  {
    if(data&0x80)
        PutPixel(x, y+8-i, SAMSUNG.color);
    else
        PutPixel(x, y+8-i, SAMSUNG.background);
    data<<=1;
    i++;
  }
}
...
void LCD_P8x16Str(uint16_t y, uint16_t x, uint8_t ch[])
{
  ...
  while (ch[j]!='\0')
  {
    ...
    for(i=0;i<8;i++){
        x++;
        LCD_WrDat(y, x, F8X16[c*16+i]);
    }
    x-=8;
    for(i=0;i<8;i++){
        x++;
        LCD_WrDat(y+8, x, F8X16[c*16+i+8]);
    }
    j++;
  }
}
...
void LCD_draw_bitmap(){
  ...
  for(i=0;i<8;i++)
    for(j=0;j<128;j++){
      LCD_WrDat(200+i*8, j, robot[128*i+j]);
    }
    SAMSUNG.color = 0x00FF00;
    for(i=0;i<8;i++)
    for(j=0;j<128;j++){
      LCD_WrDat(200+i*8, 128+48+j, robot[128*i+j]);
    }
    SAMSUNG.color = 0x0000FF;
    for(i=0;i<8;i++)
    for(j=0;j<128;j++){
      LCD_WrDat(200+i*8, 128*2+48*2+j, robot[128*i+j]);
    }
}

void LCD_test(){
    LCD_P6x8Str(0,0,"Hello,world.");
    LCD_P6x8Str(16,12,"This is for everyone!");
    LCD_P6x8Str(32,24,"U-BOOT");
    LCD_P6x8Str(48,36,"Linux");
    
    LCD_P8x16Str(64, 48, "CPU:S3C2440 FRE:800MHz");
    LCD_P8x16Str(96, 80, "Where is a will, there's a way.");
    LCD_P8x16Str(96, 80, "HPU");
